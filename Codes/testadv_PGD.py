...
def pgd_attack(model, img, y_train, epsilon, alpha, num_iter):
    
    img_pgd = img.clone().detach()
    img_pgd.requires_grad = True
    
    for i in range(num_iter):
        
        output = model(img_pgd)
        loss = getLoss(output, y_train.unsqueeze(1))
        
        model.zero_grad()
        loss.backward(retain_graph=True)
        
        img_grad = img_pgd.grad.detach()
        img_pgd = img_pgd + alpha * img_grad.sign()
        
        # Clip the perturbed image to ensure it stays within the epsilon range
        img_pgd = torch.min(torch.max(img_pgd, img - epsilon), img + epsilon)
        img_pgd = torch.clamp(img_pgd, 0, 1)
        
        # Detach the gradients to prevent accumulation
        img_pgd.detach_()
        img_pgd.requires_grad_()
    ...
    for batch_num, (leftImg, centerImg, rightImg, leftAng, centerAng, rightAng) in enumerate(valid_loader):
        ...
        for (img, y_train) in [[leftImg, leftAng], [centerImg, centerAng], [rightImg, rightAng]]:
            ...
            #y_train = y_train.unsqueeze(1) # Shape N x 1
             ...
             # Generate adversarial image using PGD attack
            img_adv = pgd_attack(model, img, y_train, epsilon, alpha, num_iter)
            
            # Predict on adversarial image
            y_pred_adv = model(img_adv)

            # Calculate loss for adversarial image
            loss_adv = getLoss(y_pred_adv, y_train.unsqueeze(1))
